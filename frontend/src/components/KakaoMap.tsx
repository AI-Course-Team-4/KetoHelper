...
type KakaoMapProps = {
  ...
  onMarkerClick?: (payload: { index: number; lat: number; lng: number; title?: string; phone?: string }) => void;
...
<svg xmlns="http://www.w3.org/2000/svg" width="${markerSize}" height="${markerSize}" viewBox="0 0 24 24">
  ...
  <circle cx="12" cy="9.5" r="3" fill="#ffffff"/>
        ...
        let positions: Array<{ lat: number; lng: number; title?: string; address?: string; phone?: string }> = [];
        if (restaurants && restaurants.length > 0) {
          const hasMissing = restaurants.some(r => typeof r.lat !== 'number' || typeof r.lng !== 'number');
          if (!hasMissing) {
            positions = restaurants.map(r => ({ lat: r.lat as number, lng: r.lng as number, title: r.name, address: r.address, phone: r.phone }));
            ...
            for (const r of restaurants) {
              ...
              if (typeof coord.lat === 'number' && typeof coord.lng === 'number') {
                positions.push({ lat: coord.lat!, lng: coord.lng!, title: r.name, address: r.address, phone: r.phone });
              }
            }
          } else {
            ...
            positions = restaurants
              ...
              .map(r => ({ lat: r.lat as number, lng: r.lng as number, title: r.name, address: r.address, phone: r.phone }));
          }
          ...
          positions = markers.map((m: { lat: number; lng: number; title?: string }) => ({ lat: m.lat, lng: m.lng, title: m.title }));
          positions = markers.map((m: { lat: number; lng: number; title?: string; phone?: string }) => ({ lat: m.lat, lng: m.lng, title: m.title, phone: m.phone }));
        } else {
          positions = [{ lat, lng }];
        }
        ...
        positions.forEach((pos, index) => {
            ...
            <div style="position:relative;pointer-events:auto;">
              ...
              ">
                <div style="padding:12px 14px;font-size:13px;color:#212121;">
                  ...
                  ${`<div style=\"color:#616161;\">${(pos as any).phone}</div>`}
                </div>
              </div>
              <div style="
          ...
          window.kakao.maps.event.addListener(marker, 'click', () => {
            ...
            if (onMarkerClick) {
              onMarkerClick({ index, lat: pos.lat, lng: pos.lng, title: (pos as any).title, phone: (pos as any).phone });
            }
          });
        });
      });

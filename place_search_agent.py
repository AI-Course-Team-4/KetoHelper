...
class PlaceSearchAgent:
    ...
    async def search_places(
        ...
        try:
            ...
            try:
                ...
                print(f"⏰ 검색 타임아웃 ({settings.search_timeout_seconds}초)")
    ...
    async def _improve_search_keywords(self, message: str) -> List[str]:
        ...
        try:
            ...
            llm_response = await asyncio.wait_for(
                ...
                timeout=settings.api_timeout_seconds  # 15초 타임아웃
            )
            ...
            print(f"  ⏰ 키워드 개선 타임아웃 ({settings.api_timeout_seconds}초) - 원본 메시지 사용")
            return [message]
    ...
    async def _generate_response(
        ...
        """검색 결과 기반 응답 생성 (기존 버전)"""
        if not results:
            return "죄송합니다. 요청하신 조건에 맞는 식당을 찾을 수 없습니다. 다른 지역이나 키워드로 다시 시도해보세요."
        
        if not self.llm:
            return f"총 {len(results)}개의 키토 친화적 식당을 찾았습니다."
        ...
        try:
            ...
            for i, restaurant in enumerate(results[:5], 1):
                ...
                restaurant_summary += f"   키토 점수: {restaurant.get('keto_score', 0)}/100\n"
                
                # RAG 결과인 경우 메뉴 정보 추가
                if restaurant.get('source') == 'rag' and restaurant.get('menu_info', {}).get('name'):
                    menu_info = restaurant.get('menu_info', {})
                    restaurant_summary += f"   추천메뉴: {menu_info.get('name', '')}"
                    if menu_info.get('price'):
                        restaurant_summary += f" ({menu_info.get('price')}원)"
                    restaurant_summary += "\n"
...
다음은 찾은 키토 친화적 식당들입니다:
{restaurant_summary}

사용자 프로필: {profile_text if profile_text else "특별한 제약사항 없음"}

위 정보를 바탕으로 친근하고 도움이 되는 식당 추천 응답을 작성해주세요.
- 키토 다이어트에 적합한 이유를 설명해주세요
- 각 식당의 특징과 추천 메뉴를 언급해주세요
- 사용자 프로필을 고려한 개인화된 조언을 포함해주세요
"""
            
            # 타임아웃 적용하여 LLM 호출
            llm_response = await asyncio.wait_for(
                self.llm.ainvoke([HumanMessage(content=response_prompt)]),
                timeout=settings.llm_timeout_seconds
            )
            return llm_response.content
            
        except asyncio.TimeoutError:
            print(f"⏰ 상세 응답 생성 타임아웃 ({settings.llm_timeout_seconds}초) - 간단한 응답으로 폴백")
...
위 식당들을 친근하게 추천해주세요. 키토 다이어트에 좋은 이유를 간단히 설명하고, 각 식당의 특징을 언급해주세요. 200자 이내로 간결하게 작성해주세요."""
            ...
            print(f"⏰ LLM 응답 생성 타임아웃 ({settings.llm_timeout_seconds}초)")

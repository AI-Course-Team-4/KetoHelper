...
def insert_sample_data():
    ...
    try:
        ...
        # 기존 데이터 확인
        try:
            existing_data = supabase.table('recipes').select('id').limit(1).execute()
            if existing_data.data:
                print("⚠️ 기존 데이터가 있습니다. 삭제 후 새로 삽입하시겠습니까? (y/n)")
                choice = input().lower()
                if choice == 'y':
                    # 기존 데이터 삭제
            ...
            print(f"⚠️ 테이블 접근 오류: {e}")
            print("💡 먼저 Supabase 대시보드에서 recipes 테이블을 생성해주세요.")
            return False

...
class RestaurantAgent:
    ...
    def _load_tools(self) -> Dict[str, Any]:
        ...
        for key, filename in self.tool_files.items():
            try:
                module_path = f"app.tools.restaurant.{filename}"
    ...
    def _get_default_prompt(self, key: str) -> str:
        ...
        try:
            if key == "search_improvement":
                from app.prompts.restaurant.search_improvement import DEFAULT_SEARCH_IMPROVEMENT_PROMPT
                return DEFAULT_SEARCH_IMPROVEMENT_PROMPT
            elif key == "search_failure":
                from app.prompts.restaurant.search_failure import DEFAULT_SEARCH_FAILURE_PROMPT
                return DEFAULT_SEARCH_FAILURE_PROMPT
            elif key == "recommendation":
                from app.prompts.restaurant.recommendation import DEFAULT_RECOMMENDATION_PROMPT

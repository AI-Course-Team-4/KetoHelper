...
- "meal_planning" (식단/레시피 생성 요청)
- "restaurant_search" (식당 검색 요청)  
- "calendar_save" (캘린더 저장 요청)
- "general" (일반 대화)

의도 분류 기준 (중요!):
- meal_planning: 식단 계획, 레시피, 요리법 **생성/만들기** 요청
  예: "불고기 레시피", "계란말이 만드는 법", "7일 식단표 만들어줘", "이번 주 메뉴 계획", "키토 식단 만들어줘"
  
- restaurant_search: 식당을 **찾기/검색** 요청
...
2. **MEAL_PLANNING**: "짜줘", "만들어줘", "계획해줘" + 식단/레시피 관련  
   → "이번 주 7일 코스 키토 식단 짜줄래?" = MEAL_PLANNING (식단 짜줘)
3. **RESTAURANT_SEARCH**: "찾아줘", "추천해줘", "어디있어" + 장소 관련
4. **GENERAL**: 순수 질문형("뭐야?", "어떻게?", "왜?") + 인사 + 기억 요청

**중요**: 
- "짜줘", "만들어줘", "계획해줘"가 있으면 MEAL_PLANNING 우선 고려

...
4. **calendar_save** - 캘린더 저장:
   ...
   - 충돌 검사 및 해결
   - 타이틀/메모 커스터마이징
   예: "캘린더에 저장", "캘린더에 저장해줘", "저장해줘", "일정 등록", "재등록", "타이틀 바꿔서 저장", "넣어줘", "넣어", "추가해줘", "추가해"
...
5. **일반 대화**: 인사, 질문, 배경 설명 등 **구체적 행위 요청 없음** → general
...
- ✅ "키토 시작하려고 해" → general (결심 표명, 요청 없음)
- ❌ "키토 다이어트 방법 알려줘" → general (질문, 구체적 요청 없음)
- ❌ "키토 하려면 뭐 먹어?" → general (질문, 구체적 요청 없음)
- ✅ "일주일 키토 식단표 짜줘" → meal_plan (명시적 요청)
- ✅ "7일치 키토 식단표를 만들고 12월 1일부터 저장해줘" → meal_plan (복합 요청: 식단표 생성 + 저장)

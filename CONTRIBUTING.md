...
git config --global alias.dev2main '!f(){ set -e; BR=$(git rev-parse --abbrev-ref HEAD); [ "$BR" != dev ] && { echo "dev 브랜치에서 실행하세요."; exit 1; }; git fetch origin; if ! git merge origin/main; then echo "⚠️ 충돌 발생: 해결 후 ① git add -A ② git commit ③ git push origin dev ④ gh pr create -B main -H dev -T .github/pull_request_template.md -w"; exit 1; fi; git push origin dev; gh pr create -B main -H dev -T .github/pull_request_template.md -w; }; f'

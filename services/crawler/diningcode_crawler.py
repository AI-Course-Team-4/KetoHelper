...
class DiningcodeCrawler(BaseCrawler):
    ...
    def _clean_address(self, address: str) -> str:
        ...
        # 1단계: 정확한 주소 패턴만 추출 (가장 중요)
        # 서울특별시 + 구 + 동/로/길 + 숫자 + 건물정보 까지만
        address_patterns = [
            # 도로명 주소: 서울특별시 강남구 테헤란로10길 21 1층
            r'(서울특별시\s+\S+구\s+\S*[로길]\d*\s+\d+(?:\s*-\s*\d+)?(?:\s+\d*층)?)',
            # 지번 주소: 서울특별시 강남구 역삼동 123-45
            r'(서울특별시\s+\S+구\s+\S*동\s+\d+(?:\s*-\s*\d+)?)',
            # 간단한 패턴: 서울특별시 강남구 테헤란로 147
            r'(서울특별시\s+\S+구\s+\S+\s+\d+)'
        ]
        ...
        # 불필요한 텍스트 제거
        unwanted_texts = [
            '지번', '도로명', '번지', '번지수', '상세주소', '건물명', '층수',
            '전화번호', '영업시간', '휴무일', '주차', '메뉴', '가격', '리뷰',
            '평점', '별점', '추천', '맛집', '음식점', '레스토랑', '카페', '바',
            '⏰', '📞', '🏪', '🍽️', '💰', '⭐', '📍', '🚗', '🅿️',
            '평일', '주말', '공휴일', '휴무', '영업', '시간', '분', '원',
            '감자탕', '부대찌개', '된장찌개', '김치찌개', '순두부', '갈비', '삼겹살',
            '치킨', '피자', '햄버거', '파스타', '스테이크', '초밥', '회', '라면',
            '뼈', '숯불', '구이', '볶음', '찜', '탕', '국', '밥', '면', '전골'
        ]
        ...
        # 3단계: 마지막 검증 - 주소가 너무 길거나 이상하면 기본 패턴만
        if len(address) > 50 or not re.search(r'서울특별시.*구.*\d+', address):
            # 최소한의 주소 정보만 추출
            basic_match = re.search(r'(서울특별시\s+\S+구\s+\S+\s+\d+)', address)
            if basic_match:
